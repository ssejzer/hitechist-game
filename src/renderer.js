...
export class Renderer {
  ...
  draw(g, time, menu = false) {
    ...
    const entities = [
      ...this.tiles
        .filter(
          (t) =>
            (isRack(t.type) || ["utility", "plant"].includes(t.type)) &&
    ...
    for (const s of g.servers) {
      const p = this.screen(s.x, s.y),
        color = s.hp <= 0 ? "#c8785d" : s.hp < 35 ? "#e7a475" : "#b7ed8e";
      ...
      ctx.setLineDash([]);
      if (!menu) {
        this.text(ctx, s.name, p.x, p.y + 30, color, 10, "center");
        this.rect(ctx, p.x - 23, p.y + 37, 46, 3, "#122920");
        this.rect(ctx, p.x - 23, p.y + 37, (46 * s.hp) / 100, 3, color);
        if (Math.hypot(g.player.x - s.x, g.player.y - s.y) < 95 && s.hp < 100)
          this.text(ctx, "[E] REPAIR", p.x, p.y - 98, "#c5dfa9", 10, "center");
      }
    }
    ...
    for (const f of g.floaters) {
      ...
      this.text(ctx, f.text, p.x, p.y, f.color, 11, "center");
    }
    ctx.globalAlpha = 1;
  ...
  minimap(g, viewWidth) {
    ...
    ctx.strokeRect(
      ...
      my +
        (clamp(g.player.y - 1000, 0, WORLD_HEIGHT - 2000) / WORLD_HEIGHT) * mh,
      (2000 / WORLD_WIDTH) * mw,
    ...
    for (const s of g.servers)
      this.rect(
        ctx,
        mx + (s.x / WORLD_WIDTH) * mw - 3,
        my + (s.y / WORLD_HEIGHT) * mh - 3,
        6,
        6,
        s.hp <= 0 ? "#d67760" : s.hp < 35 ? "#e5b06c" : "#97c77c",
      );
    this.rect(
      ctx,
      mx + (g.player.x / WORLD_WIDTH) * mw - 2,
      my + (g.player.y / WORLD_HEIGHT) * mh - 2,
      4,
      4,
      "#f2f4da",
    );
    this.text(
      ctx,
      worldLabel(g.player.x, g.player.y).split(" / ")[0],
      mx + mw / 2,
      my + mh - 5,
      "#829b76",
      7,
